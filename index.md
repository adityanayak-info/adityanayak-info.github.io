---
layout: default
title: Home
permalink: /
---

<!-- ═══════════════════════════════════════════════════════════
     PROFILE SECTION
     Left column: photo → affiliation → buttons
     Right column: name, text, research interests

     TO ADD YOUR PHOTO:
       1. Save your image as:  assets/img/photo.jpg
       2. Replace the line:    <div class="photo-initials">AN</div>
          with:                <img src="{{ '/assets/img/photo.jpg' | relative_url }}" alt="Aditya Nayak">
     ════════════════════════════════════════════════════════════ -->

<div class="profile-section">

  <!-- ── Left column: photo + affiliation + buttons ── -->
  <div class="photo-col">

    <img src="{{ '/assets/img/FIMS.jpg' | relative_url }}" alt="Aditya Nayak">
    <!-- swap the line above for your photo once ready -->

    <div class="contact-links">


      <a href="mailto:aditya.nayak@pitt.edu" aria-label="Email">
       <svg viewBox="0 0 24 24" fill="none" stroke="#123C69" stroke-width="2" width="22" height="22" aria-hidden="true"><rect x="2" y="4" width="20" height="16" rx="2"/><path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7"/></svg>
     </a>

     <a href="https://www.linkedin.com/in/adityanayak826/" target="_blank" rel="noopener" aria-label="LinkedIn">
       <svg viewBox="0 0 24 24" fill="#123C69" width="22" height="22" aria-hidden="true"><path d="M19 3a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h14m-.5 15.5v-5.3a3.26 3.26 0 0 0-3.26-3.26c-.85 0-1.84.52-2.32 1.3v-1.11h-2.79v8.37h2.79v-4.93c0-.77.62-1.4 1.39-1.4a1.4 1.4 0 0 1 1.4 1.4v4.93h2.79M6.88 8.56a1.68 1.68 0 0 0 1.68-1.68c0-.93-.75-1.69-1.68-1.69a1.69 1.69 0 0 0-1.69 1.69c0 .93.76 1.68 1.69 1.68m1.39 9.94v-8.37H5.5v8.37h2.77z"/></svg>
     </a>

     <a href="https://scholar.google.com/citations?user=2aDc8HAAAAAJ&hl=en" target="_blank" rel="noopener" aria-label="Google Scholar">
       <svg viewBox="0 0 24 24" fill="#123C69" width="22" height="22" aria-hidden="true"><path d="M12 3 1 9l4 2.18V17l7 4 7-4v-5.82L23 9zm6.36 7L12 13.36 5.64 10 12 6.64zM17 15.56l-5 2.86-5-2.86v-3.73L12 14l5-2.17z"/></svg>
     </a>
      
    </div>
  </div>

  <!-- ── Right column: text only ── -->
  <div class="profile-text">
    <h1>Aditya Nayak</h1>
    <p class="profile-subtitle">PhD Student · School of Computing and Information</p>

    <hr class="profile-rule">

    <p>I am a PhD student at the <a class="inst-link" href="https://www.sci.pitt.edu" target="_blank">University of Pittsburgh</a>, working at the intersection of Human-Computer Interaction and the sociopolitical dimensions of technology. I use participatory design methods to co-design mechanisms for community-engagement for responsible technology development.</p>

    <hr class="profile-rule">

    <p>Previously, I was an <strong>Assistant Professor of Political Science</strong> at <a class="inst-link" href="https://www.lsr.edu.in" target="_blank">Lady Shri Ram College, University of Delhi</a>, and a Research Associate on AI Governance at <a class="inst-link" href="https://www.iiitd.ac.in" target="_blank">IIIT-Delhi</a>. I hold an M.Phil. from <a class="inst-link" href="https://www.jnu.ac.in" target="_blank">Jawaharlal Nehru University</a> with a thesis on "Artificial Intelligence and the Changing Forms of Work".</p>

    <hr class="profile-rule">

    <p><strong>Research Interests:</strong> Human-Computer Interaction, Participatory Design, Public Sector AI, Responsible AI, Community-Engagement, Fairness, Accountability &amp; Transparency in AI</p>

  </div>
</div>

## My Background

<!-- ═══════════════════════════════════════════════════════════
     CAREER PATH — edit stage-name and stage-inst text below.
     You can also replace this entire block with an SVG image:
       <img src="{{ '/assets/img/background.svg' | relative_url }}" alt="My background">
     ════════════════════════════════════════════════════════════ -->

<div class="career-path">
  <div class="career-stages">
    <div class="stage">
      <div class="stage-name">Political<br>Scientist</div>
      <div class="stage-inst">BA · MA · MPhil<br>Delhi &amp; JNU</div>
    </div>
    <div class="stage-arrow">→</div>
    <div class="stage">
      <div class="stage-name">AI Humanities<br>Researcher</div>
      <div class="stage-inst">Journal of<br>AI Humanities</div>
    </div>
    <div class="stage-arrow">→</div>
    <div class="stage">
      <div class="stage-name">Faculty &amp;<br>Policy Work</div>
      <div class="stage-inst">LSR Delhi<br>+ IIIT-Delhi</div>
    </div>
    <div class="stage-arrow">→</div>
    <div class="stage">
      <div class="stage-name">HCI / SCI<br>Scholar</div>
      <div class="stage-inst">PhD · Pitt<br>2024 – present</div>
    </div>
  </div>
  <p class="career-tagline">Interdisciplinary AI &amp; Society Researcher</p>
</div>

## Education

**PhD** · School of Computing and Information, University of Pittsburgh *(Pursuing)*
Courses: Foundations in HCI; Public Interest Technology; Social Computing; Data-Centric Computing; FATE in AI; Psychology of Learning and Development.

**M.Phil.** · Centre for Comparative Politics and Political Theory, Jawaharlal Nehru University
Thesis: *Artificial Intelligence and the Changing Forms of Work*

**MA** · Department of Political Science, University of Delhi

**BA (Hons.)** · Department of Political Science, Ramjas College, University of Delhi

## Selected Honours

- <span><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="#D4A017" width="17" height="17" style="display:inline-block;vertical-align:-3px;margin-right:4px"><path d="M19 5h-2V3H7v2H5c-1.1 0-2 .9-2 2v1c0 2.55 1.92 4.63 4.39 4.94.63 1.5 1.98 2.63 3.61 2.96V17H7v2h10v-2h-4v-2.1c1.63-.33 2.98-1.46 3.61-2.96C19.08 11.63 21 9.55 21 7V6c0-1.1-.9-2-2-2zM7 10.82C5.84 10.4 5 9.3 5 8V7h2v3.82zm12-2.82c0 1.3-.84 2.4-2 2.82V7h2v1z"/></svg></span> **Awarded First Position**, [Allegheny County Case Competition (2024)](https://www.sci.pitt.edu/news/sci-phd-student-wins-allegheny-county-competition-supporting-refugees){:target="_blank"} — *Supporting Refugees with Disabilities: Data-curation and Tracking of Disability Benefits*

- <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="#D4A017" width="17" height="17" style="display:inline-block;vertical-align:-3px;margin-right:4px"><path d="M19 5h-2V3H7v2H5c-1.1 0-2 .9-2 2v1c0 2.55 1.92 4.63 4.39 4.94.63 1.5 1.98 2.63 3.61 2.96V17H7v2h10v-2h-4v-2.1c1.63-.33 2.98-1.46 3.61-2.96C19.08 11.63 21 9.55 21 7V6c0-1.1-.9-2-2-2zM7 10.82C5.84 10.4 5 9.3 5 8V7h2v3.82zm12-2.82c0 1.3-.84 2.4-2 2.82V7h2v1z"/></svg> **Awarded First Position**, [Allegheny County Case Competition (2024)](https://www.sci.pitt.edu/news/sci-phd-student-wins-allegheny-county-competition-supporting-refugees){:target="_blank"} — *Supporting Refugees with Disabilities: Data-curation and Tracking of Disability Benefits*
- **Senior Research Fellowship** University Grants Commission, 2020-2023
- **Junior Research Fellowship** University Grants Commission, 2018-2020
- **National Eligibility Test** for Assistant Professor in Political Science, India (2018)

## Grants & Fellowships

| Award | Year |
|---|---|
| SCI Fellowship, Dept. of Information, Culture and Data Stewardship, University of Pittsburgh | 2024–25 |
| International Travel Grant, Chung-Ang University, Seoul | 2019 |
